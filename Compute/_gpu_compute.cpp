...
_GPU_Compute::_GPU_Compute() :
    ...
    texture_pool_8_ui(_TexturePool(GL_RED,GL_UNSIGNED_BYTE)),
    texture_pool_32_i(_TexturePool(GL_RED_INTEGER,GL_INT,GL_R32I)),
    texture_pool_8_ui_rgba(_TexturePool(GL_RGBA,GL_UNSIGNED_BYTE))
{
...
glm::vec3 _GPU_Compute::compute_stage_angle(_Texture& input_img,_Texture& output_img)
{
    ...
    //_Texture texture_edge(nullptr,input_img.getWidth(),input_img.getHeight(),GL_RED,GL_UNSIGNED_BYTE);
    texture_pool_8_ui.textureFromPool("edge",input_img.getWidth(),input_img.getHeight());
    ...
    _Texture& texture_edge = *texture_pool_8_ui["edge"];

    _Texture texture_hough_space(nullptr,720,glm::sqrt(pow(input_img.getWidth(),2)+pow(input_img.getHeight(),2)));
    ...
    compute_canny_edge_from_sobel(texture_sobel_mag_,texture_sobel_theta_,*texture_pool_8_ui["edge"]);
    ...
    compute_threshold(*texture_pool_8_ui["edge"],texture_mask,60);
...
_Texture *_GPU_Compute::textureFromPool(QString texture_name,GLenum typ)
{
    int n = 0;
    if(texture_pool_8_ui.doesExists(texture_name)) n++;
    if(texture_pool_32_i.doesExists(texture_name)) n+=2;
    if(texture_pool_8_ui_rgba.doesExists(texture_name)) n+=4;
    return texture_pool_8_ui.textureFromPool(texture_name);
}

_Texture *_GPU_Compute::textureFromPool(QString texture_name, unsigned int w, unsigned int h,GLenum typ)
{
    return texture_pool_8_ui.textureFromPool(texture_name,w,h);
}
