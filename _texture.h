...
public:
    ...
    _Texture(char *img,uint w, unsigned int h,uint colorFormat = GL_RGBA); //texture from char pointer array
    ~_Texture();
    ...
    void setImage(char* img,uint iwidth,uint iheight);
    void setImage(char* img,uint iwidth,uint iheight,GLenum internal_format,GLenum format);//from char pointer array with updated given resolution
