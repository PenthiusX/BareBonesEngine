...
void _Tools::SaveObjModel(std::vector<float> vertsG, std::vector<unsigned int> indiceG, QString filename)
{

    QFile file(filename);
    if (file.open(QIODevice::ReadWrite)) {
        QTextStream stream(&file);
        stream << "#generated by DiamondPal" << endl;

    for (int v = 0; v < vertsG.size(); v++) {
        stream << "v ";
        stream << vertsG[v] << " ";
        v++;
        stream << vertsG[v] << " ";
...
glm::vec3 _Tools::getPointFromIndex(std::vector<float>& vertsG, unsigned int index)
{
    return glm::vec3(vertsG[index*3],vertsG[index*3+1],vertsG[index*3+2]);
}

std::vector<glm::vec3> _Tools::getTriangleFromElementsIndex(std::vector<float>& vertsG, std::vector<unsigned int>& indiceG, unsigned int index)
{
    return {
        getPointFromIndex(vertsG,indiceG[index*3]),
                getPointFromIndex(vertsG,indiceG[index*3]),
                getPointFromIndex(vertsG,indiceG[index*3])
    };
}
